...
local function wrap_git_cmd(command)
  ...
  if not ok then vim.health.warn(string.format("Could not %s: '%s'", command, err)) end
  return ok
end
...
local function do_custom_commit(prefix)
  ...
  if diff and #diff > 0 then cmd = 'commit -m "' .. prefix .. msg .. '"' end
...
vim.keymap.set('n', '<leader>gp', function() wrap_git_cmd 'push' end, { desc = '[G]it [P]ush' })
...
vim.keymap.set('n', '<leader>gcf', function() do_custom_commit 'feat' end, { desc = '[G]it [C]ommit [F]eat' })
vim.keymap.set('n', '<leader>gcc', function() do_custom_commit 'chore' end, { desc = '[G]it [C]ommit [C]hore' })
vim.keymap.set('n', '<leader>gcr', function() do_custom_commit 'refactor' end, { desc = '[G]it [C]ommit [R]efactor' })
vim.keymap.set('n', '<leader>gce', function() do_custom_commit '' end, { desc = '[G]it [C]ommit [E]mpty' })
...
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function() vim.keymap.set('n', 'q', '<Cmd>q<CR>', { buffer = true, desc = 'Close git window' }) end,
})
...
vim.keymap.set('n', '<leader>gM', function()
  select_git_branch({ prompt = 'Branch to merge in:', default = 'develop' }, function(branch_name)
    if not wrap_git_cmd('merge ' .. branch_name) then return end
  end)
...
vim.keymap.set('n', '<leader>gRs', function()
  select_git_branch({ prompt = 'Branch to rebase onto:', default = 'origin/develop' }, function(branch_name)
    ...
    for _, command in ipairs(commands) do
      if not wrap_git_cmd(command) then return end
    end
  end)
